...
export const POST = withOrganizationRoute(async ({ request, organizationId, user }) => {
  ...
  if (type !== "ticket.process") return NextResponse.json({ error: { code: "JOB_HANDLER_UNAVAILABLE", message: `${type} is not enabled in this rollout.` } }, { status: 409 });
  ...
  const input = { ticketInput: rawInput as Record<string, unknown> };
