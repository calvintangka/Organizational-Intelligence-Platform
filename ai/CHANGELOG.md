...
- **Phase A audit (measured, not assumed):** three shared normalizers (`containsSignal`, `normalizeText`, `normalizeForSignalMatching`) all fold with `[^a-z0-9\s-] -> " "`. CJK is erased entirely (`"ログインできません"` → `""`) and accented Latin words are split rather than folded (`"sesión"` → `"sesi n"`). Running one login problem in ten languages through the real pipeline: only 4/10 reach canonical `Login Issue`, and only incidentally — those four contain the English loanwords `login`/`password`. Key finding: canonical problems and lessons are already keyed by stable ids, so **memory is already language-neutral; only the lookup path is not** — no migration or re-authoring is needed.
- **Phase B** — `lib/languageDetection.ts`: deterministic, offline, provider-independent detection for all ten required languages. Script stage (kana/Hangul/Han) plus a lexical stage (weighted function words + orthography markers). Deterministic tie-break, and unusable input returns the organization default with `fallbackApplied` and confidence 0 so "assumed" is distinguishable from "detected". 20/20 across all ten languages, long and short form.
- **Phase E** — `lib/conceptVocabulary.ts`: vocabulary as CONCEPTS, not words. 10 built-in concepts × ~179 aliases across ten languages; organization concepts extend built-ins and win on id collision. All ten languages reach the same `{login, password}` concept set, with no language-suffixed concepts.
- **Phase F** — `lib/languagePolicy.ts`: configurable `customer_language` / `organization_language` / `fixed_language`, internal documentation language, and a minimum-confidence bar below which a weak detection never selects the reply language. The decision is resolved before any provider call and passed into the drafting prompt, so the provider no longer picks the language.
- **Unicode-safe folding** — `lib/textNormalization.ts` `foldForMatching` keeps `\p{L}\p{N}` from every script and folds only Latin diacritics `U+0300–U+036F`, then recomposes. Stripping all `\p{M}` would flatten Japanese `ログイン` → `ロクイン`; the probe caught exactly that.
- **Phases H/I/J/M** — Organization → Language settings card; ticket workspace shows detected language, confidence, method, reply language, and a reviewer override that re-resolves through policy. **Zero schema change** (both fields ride existing `Json` columns), and the server carries them forward when a client omits them so an older client cannot erase them — the TODO-056 failure mode prevented by design. Overhead: detection 1–9 µs, concepts ~60 µs, policy <1 µs per ticket.
- **Phases C/D/G (canonical matching, retrieval, reflection on concepts) are DESIGNED, NOT IMPLEMENTED** — see `docs/TODO-058-LANGUAGE-NEUTRAL-DESIGN.md`. The probe records today's real behavior so the gap stays measured.
- Added `probe:todo058-multilingual` (21 checks: detection, folding, concepts, policy, cross-language convergence, backward compatibility). Regression: BUG-008 retrieval + semantic, BUG-010, TODO-039, TODO-040, TODO-046, TODO-047, TODO-048, TODO-050, TODO-051, TODO-052, TODO-053, TODO-055, TODO-056, mature retrieval — all pass. TODO-025G fails on `validations.length === 140` (now 141) — proven pre-existing by `git stash`, caused by a human approval committed through the app on the demo org, identical on a clean tree. `tsc`, strict-unused, and production build clean. Zero database writes.
...
## [2026-07-15] TODO-004 Batch 5.6 post-import verification
**Layer:** persistence migration
**What changed:** Added the dedicated verify/readback route, persisted structured verification reports, normalized PostgreSQL-to-domain digest comparison, source-ID coverage checks, nested lesson/version checks, audit-history checks, ticket-sequence safety checks, conflict reporting, and safe `imported` → `verifying` → `verified` transitions.
**Safety boundary:** Verification reads only the immutable package and organization-scoped PostgreSQL state. It reports extras without deleting them, never repairs drift automatically, never writes business resources or localStorage, and leaves failed batches unverified for manual repair and retry.
**Schema:** Added `verificationReport` JSONB and `verificationError` to `MigrationImportBatch` in migration `20260715213000_add_migration_verification_report`.
**Verification:** Disposable migration verification and live HTTP probes passed clean verification, all resource digests, nested lesson/version and audit checks, retry no-op, drift failure/repair, missing memory, lower/higher sequence handling, extra-row policy, open conflicts, cross-organization protection, and unchanged business counts. Migration, persistence, Prisma, typecheck, build, and diff checks passed.
**Open items:** Batch 5.8 owns per-organization authority cutover; mature Maesa/FastDrop/Pramana migration remains unperformed.
