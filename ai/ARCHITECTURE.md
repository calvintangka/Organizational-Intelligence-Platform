...
An opt-in read-only path is available when `NEXT_PUBLIC_OIP_PERSISTENCE_MODE=server`: `app/page.tsx` -> `ServerPersistenceAdapter` -> GET-only organization API -> `lib/server/persistenceService.ts` -> lazy server-only Prisma -> PostgreSQL. The server service maps database rows into existing domain shapes, checks organization existence, and applies `organizationId` in each database query. It has no create/update/delete operation and never falls back to localStorage.

A PostgreSQL + Prisma foundation exists in `prisma/schema.prisma` and `lib/server/prisma.ts`. It is dormant in the default local mode, and no browser state is migrated or dual-written. The server read path is available only by explicit mode selection; no database write path exists yet. The schema makes organization ownership a required foreign-key relation for every organization-owned durable model and scopes ticket IDs with a composite organization/ticket constraint. `DATABASE_URL` is environment-provided; until it is configured and a later write/migration batch is explicitly performed, the database path makes no database mutation and changes no current local data.
