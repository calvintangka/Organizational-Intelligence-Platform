...
The active runtime path is `app/page.tsx` -> `PersistenceAdapter` -> `LocalStorageAdapter` -> the existing localStorage modules. The profile/list state remains the existing two-key browser model, and organization-owned resource operations retain explicit organization IDs. `LocalStorageAdapter` delegates migration preparation, loading, saving, reset, deletion, ticket counters, and ticket ID allocation to the hardened modules; it does not duplicate their logic.

A dormant PostgreSQL + Prisma foundation exists in `prisma/schema.prisma` and `lib/server/prisma.ts`, but no current adapter imports the database client and no browser state is migrated or dual-written. The future path is `PersistenceAdapter` -> `ServerPersistenceAdapter` -> API -> PostgreSQL; that server adapter and API are not implemented. The schema makes organization ownership a required foreign-key relation for every organization-owned durable model and scopes ticket IDs with a composite organization/ticket constraint. `DATABASE_URL` is environment-provided; until a future migration/adapter batch is explicitly performed, this foundation makes no database connection and changes no current data.
