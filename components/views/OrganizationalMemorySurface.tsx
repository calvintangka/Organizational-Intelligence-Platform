        ...
        <div className={cardClass(darkMode)}>
          ...
          {source && (
            <div className="space-y-5">
              ...
              {prepared && <div className={`rounded-xl border p-4 ${darkMode ? "border-amber-700/50 bg-amber-900/10" : "border-amber-200 bg-amber-50"}`}><div className="flex flex-wrap items-center justify-between gap-2"><div><p className={`text-xs font-bold uppercase tracking-wide ${darkMode ? "text-amber-300" : "text-amber-700"}`}>Prepared for human validation</p><h3 className={`mt-1 font-bold ${darkMode ? "text-white" : "text-[#111827]"}`}>{prepared.candidate.proposedContent.canonicalLearning?.title ?? prepared.candidate.proposedContent.canonicalProblemTitle}</h3></div><span className={`rounded-full px-2 py-1 text-xs font-semibold ${darkMode ? "bg-amber-900/40 text-amber-200" : "bg-white text-amber-800"}`}>Not trusted yet</span></div>{prepared.candidate.proposedContent.canonicalLearning ? <div className="mt-3 space-y-3 text-sm"><div><p className={`text-xs font-semibold uppercase tracking-wide ${muted(darkMode)}`}>Canonical lesson</p><p className={darkMode ? "text-slate-100" : "text-slate-800"}>{prepared.candidate.proposedContent.canonicalLearning.lesson}</p></div><div className="grid gap-3 md:grid-cols-2"><div><p className={`text-xs font-semibold uppercase tracking-wide ${muted(darkMode)}`}>Observed problem</p><p className={darkMode ? "text-slate-200" : "text-slate-700"}>{prepared.candidate.proposedContent.canonicalLearning.problem}</p></div><div><p className={`text-xs font-semibold uppercase tracking-wide ${muted(darkMode)}`}>Root-cause principle</p><p className={darkMode ? "text-slate-200" : "text-slate-700"}>{prepared.candidate.proposedContent.canonicalLearning.rootCause}</p></div><div><p className={`text-xs font-semibold uppercase tracking-wide ${muted(darkMode)}`}>Solution / action principle</p><p className={darkMode ? "text-slate-200" : "text-slate-700"}>{prepared.candidate.proposedContent.canonicalLearning.solution}</p></div><div><p className={`text-xs font-semibold uppercase tracking-wide ${muted(darkMode)}`}>Applicability scope</p><p className={darkMode ? "text-slate-200" : "text-slate-700"}>{prepared.candidate.proposedContent.canonicalLearning.scope}</p></div></div><div><p className={`text-xs font-semibold uppercase tracking-wide ${muted(darkMode)}`}>Exclusions / caution</p><ul className={`list-disc space-y-1 pl-5 ${darkMode ? "text-slate-200" : "text-slate-700"}`}>{prepared.candidate.proposedContent.canonicalLearning.exclusions.map((entry) => <li key={entry}>{entry}</li>)}</ul></div></div> : <p className={`mt-2 text-sm ${darkMode ? "text-slate-200" : "text-slate-700"}`}>{prepared.candidate.proposedContent.solution}</p>}<p className={`mt-2 text-xs ${muted(darkMode)}`}>{prepared.reflection.rationale}</p><label className="mt-4 block"><span className={labelClass(darkMode)}>Validation rationale</span><textarea className={`${inputClass(darkMode)} min-h-20`} value={validationRationale} onChange={(event) => setValidationRationale(event.target.value)} /></label><button type="button" disabled={busy || !validationRationale.trim()} onClick={() => { void validateLearning(); }} className="mt-3 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">Validate learning</button></div>}
            </div>
          )}
        </div>
      )}
        ...
        <div className={cardClass(darkMode)}>
          ...
          {inspection && (
            <div className="space-y-5">
              ...
              {inspection.knowledgeItem.canonicalLearning && <div className="rounded-xl border border-blue-900/60 bg-blue-950/30 p-4"><p className="text-xs font-bold uppercase tracking-wide text-blue-300">Canonical lesson</p><p className="mt-2 text-sm text-slate-100">{inspection.knowledgeItem.canonicalLearning.lesson}</p><div className="mt-3 grid gap-3 text-sm md:grid-cols-2"><div><p className={`text-xs font-semibold uppercase tracking-wide ${muted(darkMode)}`}>Root-cause principle</p><p className="mt-1 text-slate-200">{inspection.knowledgeItem.canonicalLearning.rootCause}</p></div><div><p className={`text-xs font-semibold uppercase tracking-wide ${muted(darkMode)}`}>Solution / action principle</p><p className="mt-1 text-slate-200">{inspection.knowledgeItem.canonicalLearning.solution}</p></div><div><p className={`text-xs font-semibold uppercase tracking-wide ${muted(darkMode)}`}>Applicability scope</p><p className="mt-1 text-slate-200">{inspection.knowledgeItem.canonicalLearning.scope}</p></div><div><p className={`text-xs font-semibold uppercase tracking-wide ${muted(darkMode)}`}>Exclusions / caution</p><ul className="mt-1 list-disc space-y-1 pl-5 text-slate-200">{inspection.knowledgeItem.canonicalLearning.exclusions.map((entry) => <li key={entry}>{entry}</li>)}</ul></div></div></div>}
              <div className="grid gap-4 md:grid-cols-2">
                <div>
