...
import { GovernedActionsPanel } from "@/components/views/GovernedActionsPanel";
...
export function OperationsView({ organizationId, darkMode, accentColor }: { organizationId: string; darkMode: boolean; accentColor: string }) {
  ...
  return (
    <div className={`mx-auto max-w-[1500px] p-4 md:p-6 ${darkMode ? "text-slate-100" : "text-slate-800"}`}>
      ...
      <GovernedActionsPanel organizationId={organizationId} darkMode={darkMode} accentColor={accentColor} />
      ...
      <section className={`mt-5 rounded-2xl border p-5 ${panel}`}><h2 className={`font-bold ${heading}`}>Connector operations</h2><p className={`mt-1 text-xs ${muted}`}>Tenant-scoped installations and safe connector health. Credentials and raw event payloads are never shown.</p><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{(snapshot?.connectors ?? []).map((connector) => <div key={String(connector.id)} className="rounded-xl border border-slate-200/20 p-3"><div className="flex items-center justify-between gap-2"><p className="font-semibold">{value(connector.name)}</p><span className={`rounded-full px-2 py-1 text-xs font-semibold ${tone(connector.status)}`}>{value(connector.status)}</span></div><p className={`mt-2 text-xs ${muted}`}>{value(connector.connectorType)} · {value(connector.eventCount, "0")} events · {value(connector.mappingCount, "0")} mappings</p>{connector.lastFailureSafe && <p className="mt-2 text-xs text-rose-600">{value(connector.lastFailureSafe)}</p>}</div>)}</div>{!snapshot?.connectors.length && <p className={`mt-3 text-sm ${muted}`}>No connector installations are configured for your accessible organizations.</p>}</section>
