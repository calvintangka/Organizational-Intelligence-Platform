...
- The OIP v2 memory foundation now supports a domain-neutral organizational-experience entry path in addition to the Support adapter; broader first-party evidence producers and connector-specific adapters remain future work.
- Reuse outcomes and challenge decisions are durable and organization-scoped, but challenge review is human-only in this release. Automatic evidence decay, retirement scheduling, bulk challenge review, and policy-driven scope inference are deferred.
- An open challenge remains retrievable for inspection while automation eligibility is fail-closed. The Knowledge surface supports single-challenge review and scope updates; bulk challenge management and policy-driven scope inference remain deferred.
...
## Organizational Memory retrieval and reliability boundary

Domain-neutral Organizational Memory retrieval currently uses deterministic, inspectable token and problem-facet signals. The supported path handles bounded wording variation, including synchronization and network-transition language, while failing closed when a candidate-specific transition scope is not evidenced. It does not provide embeddings, vector search, generalized RAG, or connector-backed semantic recall.

Reliability is projected separately from lifecycle and governance. A validated memory may remain active while reliability is 0; the product presents that state as requiring human review and does not treat it as unrestricted automation authority. Reuse outcomes require a stable logical submission identity and retain their Source/work and Evidence context in inspection history.
