...
export { sha256, stableStringify } from "@/lib/persistence/migrationExportDigest";
import {
...
export { computeMigrationExportDigests, countMigrationExportResources, migrationExportDigestMetadata };
...
export async function exportOrganizationSnapshot(organizationId: string): Promise<MigrationExportResult> {
  ...
  try {
    ...
    const counts = countMigrationExportResources(resources);
    const metadata = migrationExportDigestMetadata({
      organizationId,
    ...
    const digests = await computeMigrationExportDigests(resources, metadata);
